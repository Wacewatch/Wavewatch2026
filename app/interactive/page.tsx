...
export default function InteractivePage() {
    ...
    const checkProfile = async () => {
      ...
      try {
        const [interactiveResult, userProfileResult] = await Promise.all([
          supabase.from("interactive_profiles").select("*").eq("user_id", user.id).maybeSingle(),
          supabase
            .from("user_profiles")
  ...
  }, [user, loading, router]) // Add router to dependencies
      ...
      <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-yellow-500/50 rounded-2xl max-w-2xl w-full p-8 shadow-2xl">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-6 mb-6">
            ...
            <p className="text-gray-300 leading-relaxed mb-4">
              Merci d'être <span className="text-yellow-400 font-semibold">respectueux</span> et de faire preuve de{" "}
              <span className="text-yellow-400 font-semibold">compréhension</span> durant cette phase.
            </p>
            <p className="text-gray-300 leading-relaxed">
              Vos <span className="text-blue-400 font-semibold">retours</span> et votre{" "}
              <span className="text-blue-400 font-semibold">comportement</span> aideront à améliorer l'expérience
              finale.
            </p>
          </div>

          <p className="text-center text-lg text-white mb-6 font-medium">Bonne exploration ! 🎮</p>
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-b from-blue-900 via-purple-900 to-black flex items-center justify-center p-2 sm:p-4 overflow-y-auto z-50">
        <div className="bg-black/40 backdrop-blur-2xl p-3 sm:p-4 md:p-8 rounded-3xl max-w-6xl w-full my-2 sm:my-4 border-2 border-white/20 shadow-2xl max-h-[98vh] overflow-y-auto">
          <h1 className="text-xl sm:text-2xl md:text-4xl font-bold text-white mb-1 sm:mb-2 text-center">
            Créez Votre Avatar
          </h1>
          <p className="text-white/60 text-center mb-3 sm:mb-4 md:mb-6 text-xs sm:text-sm md:text-base">
            ...
            <div className="space-y-3 sm:space-y-4 md:space-y-6 max-h-none lg:max-h-[600px] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
              <div>
                <label className="text-white font-semibold mb-2 block flex items-center gap-2 text-xs sm:text-sm md:text-base">
                  <div className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-blue-500 rounded-full flex items-center justify-center text-xs">
                    1
                  </div>
                  Couleur du Corps
                </label>
              ...
              <div>
                <label className="text-white font-semibold mb-2 md:mb-3 block flex items-center gap-2 text-sm md:text-base">
                  <div className="w-5 h-5 md:w-6 md:h-6 bg-purple-500 rounded-full flex items-center justify-center text-xs">
                    2
                  </div>
                  Teint de Peau
                </label>
              ...
              <div>
                <label className="text-white font-semibold mb-2 md:mb-3 block flex items-center gap-2 text-sm md:text-base">
                  <div className="w-5 h-5 md:w-6 md:h-6 bg-green-500 rounded-full flex items-center justify-center text-xs">
                    3
                  </div>
                  Style de Cheveux
                </label>
              ...
              {avatarStyle.hairStyle !== "bald" && (
                <div>
                  <label className="text-white font-semibold mb-2 md:mb-3 block flex items-center gap-2 text-sm md:text-base">
                    <div className="w-5 h-5 md:w-6 md:h-6 bg-yellow-500 rounded-full flex items-center justify-center text-xs">
                      4
                    </div>
                    Couleur de Cheveux
                  </label>
              ...
              <div>
                <label className="text-white font-semibold mb-2 md:mb-3 block flex items-center gap-2 text-sm md:text-base">
                  <div className="w-5 h-5 md:w-6 md:h-6 bg-orange-500 rounded-full flex items-center justify-center text-xs">
                    5
                  </div>
                  Visage (Smiley)
                </label>
              ...
              <div>
                <label className="text-white font-semibold mb-2 md:mb-3 block flex items-center gap-2 text-sm md:text-base">
                  <div className="w-5 h-5 md:w-6 md:h-6 bg-red-500 rounded-full flex items-center justify-center text-xs">
                    6
                  </div>
                  Accessoires
                </label>
                    ...
                    <button
                      ...
                      className={`p-3 md:p-4 rounded-xl border-2 transition-all active:scale-95 ${
                        avatarStyle.accessory === acc.id
                          ? "bg-red-500 border-white scale-105"
                          : "bg-white/10 border-white/20 text-white/80"
                      }`}
                    >
